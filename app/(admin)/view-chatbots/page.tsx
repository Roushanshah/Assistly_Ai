...
async function ViewChatbots() {
    ...
    //get the chatbot for users
    const {data: {chatbotsByUser}} = await serverClient.query<GetChatbotByUserData, GetChatbotByUserDataVariables>({
        ...
        {sortedChatbotsByUser.length === 0 && (
            <div>
                ...
                <Link href="/create-chatbot">
                    <Button className="bg-[#7c84cc] text-white p-3 rounded-md mt-5">
                            Create Chatbot
                    </Button>
                </Link>
            </div>
